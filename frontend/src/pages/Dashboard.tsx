            ...
            {managed.map((item: any) => (
              item.type === "series" ? (
                ...
                <div key={item.request_id} className="request-card managed-card">
                  ...
                  <div className="managed-footer">
                    <span className="rtag">{(() => { const vc = item.release_count + (item.processed_count || 0); return vc > 0 ? `${vc} version${vc !== 1 ? "s" : ""}${item.release_count > 0 ? " · " + formatSize(item.total_size_mb) : " · In Library"}` : `· ${item.status}`; })()}</span>
