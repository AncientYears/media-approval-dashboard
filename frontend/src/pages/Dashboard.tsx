            ...
            {managed.map((item: any) => (
              item.type === "series" ? (
                ...
                <div key={item.request_id} className="request-card managed-card">
                  ...
                  <div className="managed-footer">
                    <span className="rtag">{item.release_count > 0 ? `${item.release_count} version${item.release_count !== 1 ? "s" : ""} · ${formatSize(item.total_size_mb)}` : item.processed_count > 0 ? `${item.processed_count} processed file${item.processed_count !== 1 ? "s" : ""} · In Library` : item.status === 'COMPLETED' ? '· In Library' : `· ${item.status}`}</span>
